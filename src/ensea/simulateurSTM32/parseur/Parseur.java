...
public class Parseur {
	...
	public String obtenirParam(){
		...
		passerEspace();
                ...
                while(indice<texte.length() && (s+texte.charAt(indice)).matches("#?-?[A-Za-z0-9_\\[\\]]*"))
                //while(indice<texte.length() && (s+texte.charAt(indice)).matches("^[ \t]*"))
		{
                    if (!(""+texte.charAt(indice)).matches("[\\[\\]]")){
			s+=texte.charAt(indice);
			}
                    indice++;
		...
		while(resteParams)
		{
			...
			if(p1.matches("[rR][0-9]+"))
			{
				...
				instru.ajouterMot(m);
                        ...
                        //AJOUT AG BEG
                        p1=obtenirParam();
                        if(p1.matches("[rR][0-9][(][rR][0-9][rR][0-9][)]"))
                        {
                                MotReg m=new MotReg(0);
			...
			else if(p1.matches("#-?[0-9]+"))
			{
				...
				zoneErreur.setText(zoneErreur.getText()+"ligne "+ligne+" : Paramètre non identifié:"+p1+"\n");
