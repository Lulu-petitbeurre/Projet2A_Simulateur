...
/*            //#9Instruction("ldr", new MotConst(7, "0101100",1),new MotReg(3,4),new MotReg(3,3),new MotReg(3,2)));
        if(downloadedInstructions.get(numInstrE).getListeParams().get(0).getValBin().compareTo("0101100")==0){
            ...
            //modif DG 2016 et LM 2017
            //#9 Instruction("ldr", new MotConst(10, "0110100000",1),new MotReg(3,3),new MotReg(3,2)));
        if(downloadedInstructions.get(numInstrE).getListeParams().get(0).getValBin().compareTo("0110100000")==0){
            ...
            motReg=(MotReg) instr.getListeParams().get(2);
            n=motReg.getVal();
            System.out.println("  EXECUTE instr:"+" R"+t+"<= [R"+n+"]");
            //theRegisters.R[d]=theRegisters.R[n]+theRegisters.R[m];
            long address=0;
            address=((theRegisters.R[n])/4)*4;
            if((address>=theROM.offsetAdr)&&(address<theROM.offsetAdr+theROM.sizeInWord*4)){
        ...
        if(downloadedInstructions.get(numInstrE).getListeParams().get(0).getValBin().compareTo("0101000")==0){
            System.out.println("  EXECUTE instr"+numInstrE+":"+"type #9 STR Rt,[Rn]");
            instr=downloadedInstructions.get(numInstrE);
            motReg=(MotReg) instr.getListeParams().get(1);
            t=motReg.getVal();
            motReg=(MotReg) instr.getListeParams().get(2);
            n=motReg.getVal();
            System.out.println("  EXECUTE instr:mem[R"+n+"]<="+" R"+t);
            long address=0;
            address=((theRegisters.R[n])/4)*4;
            if((address>=theRAM.offsetAdr)&&(address<theRAM.offsetAdr+theRAM.sizeInWord*4)){
        ...
        //#13bis Instruction("str", new MotConst(10, "0110000000",1),new MotReg(3,3),new MotReg(3,2))
        if(downloadedInstructions.get(numInstrE).getListeParams().get(0).getValBin().compareTo("0110000000")==0){
